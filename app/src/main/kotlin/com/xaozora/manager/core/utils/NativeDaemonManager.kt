...
object NativeDaemonManager {
    ...
    suspend fun extractAndStartDaemon(context: Context, enableAutd: Boolean? = null) = withContext(Dispatchers.IO) {
        daemonMutex.withLock {
            ...
            val battmonDir = File(context.filesDir, "battmon")
            if (!battmonDir.exists()) battmonDir.mkdirs()
            val batteryLogPath = File(battmonDir, "battery_logger.jsonl").absolutePath
