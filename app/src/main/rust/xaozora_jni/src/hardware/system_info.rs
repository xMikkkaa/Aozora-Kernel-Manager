...
fn get_soc_info() -> String {
    ...
    if board != "-" { board } else { get_prop("ro.hardware") }
}
...
fn get_deep_sleep() -> String {
    let mut ts_boot = libc::timespec { tv_sec: 0, tv_nsec: 0 };
    let mut ts_mono = libc::timespec { tv_sec: 0, tv_nsec: 0 };
    unsafe {
...
pub fn fetch_system_info() -> SystemInfo {
    ...
    let load_avg = if !load_avg_raw.is_empty() {
        load_avg_raw.split_whitespace().take(3).collect::<Vec<&str>>().join(" ")
    } else {
        "-".to_string()
    };
    
    let entropy = read_system_file("/proc/sys/kernel/random/entropy_avail").trim().to_string();
    
    let uptime_s = read_system_file("/proc/uptime").split_whitespace().next().unwrap_or("0").parse::<f64>().unwrap_or(0.0);
    
    let wg_version = read_system_file("/sys/module/wireguard/version").trim().to_string();
    let wireguard = if !wg_version.is_empty() { wg_version } else { "Unsupported".to_string() };
    
    let open_gl_output = execute_cmd_and_get_output("dumpsys SurfaceFlinger | grep -i GLES").trim().to_string();
    ...
    let gov_str = read_system_file("/sys/devices/system/cpu/cpu0/cpufreq/scaling_governor").trim().to_string();
    let governor = if !gov_str.is_empty() { gov_str } else { "Unknown".to_string() };
    
    let mut charge_full = 0f32;
    let mut charge_full_design = 0f32;
    let bat_paths = ["bms", "battery", "BAT0"];
    for p in &bat_paths {
        let full_str = read_system_file(&format!("/sys/class/power_supply/{}/charge_full", p)).trim().to_string();
        let design_str = read_system_file(&format!("/sys/class/power_supply/{}/charge_full_design", p)).trim().to_string();
    ...
    if let Ok(content) = std::fs::read_to_string("/data/data/com.xaozora.manager/files/battmon/battery_stats.json") {
    ...
    if charge_full_design > 0f32 {
        let design_capacity_mah = if charge_full_design > 10000.0 { (charge_full_design / 1000.0) as i32 } else { charge_full_design as i32 };
        let current_capacity = if learned_capacity > 0f32 {
            if learned_capacity > 10000f32 { learned_capacity / 1000f32 } else { learned_capacity }
        } else {
            if charge_full > 10000f32 { charge_full / 1000f32 } else { charge_full }
        };

        let mut health_raw = current_capacity / design_capacity_mah as f32 * 100.0 ;
        if health_raw > 100.0 { health_raw = 100.0; }
        if health_raw < 0.0 { health_raw = 0.0; }
        
        let health_category = if health_raw >= 80.0 { "Good" } else if health_raw >= 60.0 { "Fair" } else { "Poor" };
    ...
    let selinux = if selinux.is_empty() { "-".to_string() } else { selinux };
...
pub fn poll_hardware() -> RealTimeMetrics {
    ...
    if let Some(cpu_line) = stat.lines().find(|l| l.starts_with("cpu ")) {
        ...
        let idle = if parts.len() > 4 { parts[4].parse::<u64>().unwrap_or(0) } else { 0 };
    ...
    for i in 0..8 {
        let f_str = read_system_file(&format!("/sys/devices/system/cpu/cpu{}/cpufreq/scaling_cur_freq", i)).trim().to_string();
        let max_str = read_system_file(&format!("/sys/devices/system/cpu/cpu{}/cpufreq/scaling_max_freq", i)).trim().to_string();
        ...
        if f_str.is_empty() {
            ...
            let formatted_freq = if f_str.len() > 3 { format!("{} MHz", &f_str[..f_str.len()-3]) } else { format!("{} MHz", f_str) };
            core_freqs.push(formatted_freq);
            
            let prog = if max_val > 0 { (freq_val as f32 / max_val as f32).clamp(0.0, 1.0) } else { 0.0 };
            core_progress.push(prog);
        }
    }

    // GPU Load & Freq
    let g_load_str = read_system_file("/sys/class/kgsl/kgsl-3d0/gpu_busy_percentage").trim().to_string();
    let raw_load_pct = g_load_str.replace("%", "").trim().parse::<f32>().unwrap_or(0.0);
    
    let mut g_freq_val = read_system_file("/sys/class/kgsl/kgsl-3d0/gpuclk").trim().parse::<u64>().unwrap_or(0);
    if g_freq_val == 0 {
        g_freq_val = read_system_file("/sys/class/kgsl/kgsl-3d0/devfreq/cur_freq").trim().parse::<u64>().unwrap_or(0);
    }
    
    let mut max_freq_val = read_system_file("/sys/class/kgsl/kgsl-3d0/max_gpuclk").trim().parse::<u64>().unwrap_or(0);
    if max_freq_val == 0 {
        max_freq_val = read_system_file("/sys/class/kgsl/kgsl-3d0/devfreq/max_freq").trim().parse::<u64>().unwrap_or(710000000);
    }
    
    let mut gpu_load = 0.0;
    if max_freq_val > 0 {
        gpu_load = ((raw_load_pct / 100.0) * (g_freq_val as f32 / max_freq_val as f32)).clamp(0.0, 1.0);
    }
    let gpu_freq = if g_freq_val > 0 {
        if g_freq_val > 1000000 { format!("{} MHz", g_freq_val / 1000000) }
        else if g_freq_val > 1000 { format!("{} MHz", g_freq_val / 1000) }
        else { format!("{} MHz", g_freq_val) }
    } else { "-- MHz".to_string() };
    ...
    let ram_progress = if m_total > 0 { m_used as f32 / m_total as f32 } else { 0.0 };
    ...
    let swap_progress = if s_total > 0 { s_used as f32 / s_total as f32 } else { 0.0 };
    ...
    for line in bat_dump.lines() {
        ...
        if info.starts_with("level: ") {
            ...
            t_temp = info.replace("temperature: ", "").parse::<f32>().unwrap_or(0.0) / 10.0;
        }
    }
    let battery_level = t_level;
    let battery_temp = format!("{:.1}°C", t_temp);
    
    let current_raw = read_system_file("/sys/class/power_supply/battery/current_now").trim().to_string();
    let battery_current = if !current_raw.is_empty() {
        if let Ok(ma) = current_raw.parse::<i64>() {
            if ma.abs() > 10000 { format!("{}mA", ma / 1000) } else { format!("{}mA", ma) }
        } else { "0mA".to_string() }
    } else { "-272mA".to_string() };
...
#[no_mangle]
pub extern "system" fn Java_com_xaozora_manager_core_utils_SystemInfoUtils_fetchSystemInfoJson<'local>(
    env: JNIEnv<'local>,
    _class: JClass,
) -> jstring {
...
#[no_mangle]
pub extern "system" fn Java_com_xaozora_manager_core_utils_SystemInfoUtils_pollHardwareJson<'local>(
    env: JNIEnv<'local>,
    _class: JClass,
) -> jstring {
